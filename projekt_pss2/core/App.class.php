...
class App {
    ...
    public static function createAndInitialize(&$config) {
        ...
        if (self::$config->clean_urls) {
            ...
            //var_dump($_PARAMS);
            $arr = explode("/", $req_uri);
            // check if last index in URI is numeric (some id)
            // if so, set action to full path without id on end
            // i.e. /rezerwacja/typ/1 will be formatted to /rezerwacja/typ and passed to rezerwacja/typ route
            // and /car/1 will be formatted to /car and passed to car route
            if(count($arr) > 1 && is_numeric($arr[intval(count($arr) - 1)])) {
                unset($arr[intval(count($arr) - 1)]);
                $_PARAMS = implode("/", $arr);
            //    var_dump($_PARAMS);
                App::getRouter()->setAction($_PARAMS);
            } else { //if simple route i.e /katalog is detected pass full path to route katalog
                App::getRouter()->setAction($_PARAMS);
            }
